...
def find_closest_mard_color(pixel_rgb):
    min_distance = float('inf')
    closest_code = 'H2'
    closest_rgb = (255, 255, 255)
    
    for code, rgb in MARD_221_COLORS.items():
        dist = color_distance(pixel_rgb, rgb)
...
def downsample_long_edge_size(img: Image.Image, target_size: int = 52) -> Image.Image:
    w, h = img.size
    long_side = max(w, h)
...
def restore_original_size_mosaic(img_low: Image.Image, origin_w, origin_h) -> Image.Image:
    img_mosaic = img_low.resize((origin_w, origin_h), Image.Resampling.NEAREST)
    return img_mosaic

def map_to_mard_colors(img_low: Image.Image) -> dict:
    color_map = {}
    pixels = img_low.load()
    w, h = img_low.size
    ...
    for y in range(h):
        for x in range(w):
            ...
            code, mapped_rgb = find_closest_mard_color(original_rgb)
            color_map[(x, y)] = {'code': code, 'original': original_rgb, 'mapped': mapped_rgb}
    
    return color_map

def create_color_mapped_image(img_low: Image.Image, color_map: dict, scale_factor: int = 20) -> Image.Image:
    w, h = img_low.size
...
def main(args):
    img_origin = Image.open(args.input).convert("RGB")
    ...
    img_low_scale = downsample_long_edge_size(img_origin, args.size)
    print(f"降采样尺寸：{img_low_scale.size}")
    
    color_map = map_to_mard_colors(img_low_scale)
    ...
    img_result = restore_original_size_mosaic(img_low_scale, ori_w, ori_h)
    
    scale_factor = max(20, min(40, 1000 // max(img_low_scale.size)))
    img_labeled = create_color_mapped_image(img_low_scale, color_map, scale_factor)
    ...
    img_low_scale.save(low_res_path)
    img_result.save(mosaic_path)
    img_labeled.save(color_map_path)
